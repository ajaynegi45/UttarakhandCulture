...
import { rulers, eraGrouped } from "@/data/rulers";
import { Crown, ArrowRight, Sparkles, Mountain, Landmark } from "lucide-react";
      ...
      <section className="relative pt-20 pb-16 px-4 md:px-6 overflow-hidden bg-primary">
        ...
        <div className="max-w-5xl mx-auto relative z-30">
          ...
          <div className="space-y-6 md:space-y-8">
            ...
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 pt-4">
                ...
                >
                  ...
                  <div className="text-xs md:text-sm text-white/70 uppercase tracking-wider">Dynasties</div>
                </motion.div>
                ...
                >
                  ...
                  <div className="text-xs md:text-sm text-white/70 uppercase tracking-wider">Years</div>
                </motion.div>
                ...
                >
                  ...
                  <div className="text-xs md:text-sm text-white/70 uppercase tracking-wider">Legacy</div>
                </motion.div>
              </div>
            </motion.div>
            ...
            >
              <Button size="lg" className="px-8 md:px-12 py-6 md:py-7 bg-white text-primary hover:bg-amber-100 hover:scale-105 transition-all font-bold text-base md:text-lg shadow-lg hover:shadow-2xl">
              ...
              <Section>
                ...
                >
                  <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-secondary/10 border border-secondary/20">
                    <span className="text-xs uppercase tracking-widest font-semibold text-secondary">{era}</span>
                  </div>
                    ...
                    return (
                      ...
                      >
                        <Link href={`/rulers/${ruler.id}`} className="block">
                            <Card className="overflow-hidden shadow-sm hover:shadow-lg border-border/50 transition-all duration-300 hover:-translate-y-1 cursor-pointer">
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-0">
                                {/* Image Section */}
                                <div className="md:col-span-1 relative h-80 md:h-auto overflow-hidden bg-gray-200">
                                  <Image fill
                                    src={rulerImage}
                                    alt={ruler.name}
                                ...
                                {/* Content Section */}
                                <CardContent className="md:col-span-2 p-4 md:p-8 lg:p-12 flex flex-col justify-center space-y-4 md:space-y-6">
                                  {/* Period Badge */}
                                  <div className="flex flex-col md:flex-row md:items-center gap-1.5 md:gap-3">
                                    <span className="inline-flex items-center gap-2 px-2.5 md:px-3 py-1 rounded-full bg-secondary/10 text-secondary text-xs font-bold uppercase tracking-wider w-fit">
                                      {ruler.period}
                                    </span>
                                  ...
                                  <div className="space-y-1">
                                    <span className="font-hand text-secondary text-lg md:text-2xl block">Era of</span>
                                    <h3 className="text-2xl md:text-3xl lg:text-4xl font-serif font-bold text-primary group-hover:text-secondary transition-colors flex items-center gap-2">
                                      {ruler.name}
                                      <ArrowRight className="w-4 md:w-5 h-4 md:h-5 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all" />
                                    </h3>
                                  </div>
                                  ...
                                  {/* Highlights */}
                                  <div className="space-y-1.5 md:space-y-2 pt-3 md:pt-4 border-t border-border/30">
                                    {ruler.highlights.slice(0, 3).map((highlight, h_idx) => (
                                      <div key={h_idx} className="flex items-start gap-2 md:gap-3">
                                        <div className="w-1.5 h-1.5 bg-secondary rounded-full mt-1 md:mt-2 flex-shrink-0" />
      ...
      <Section className="py-20 bg-muted/30">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-secondary/10 border border-secondary/20">
            ...
            <span className="text-xs uppercase tracking-widest font-semibold text-secondary">Enduring Legacy</span>
          </div>
          ...
          <p className="text-lg text-muted-foreground leading-relaxed">
            Each ruler, each dynasty, each empire left their mark on these sacred mountains. Their decisions shaped the temples we worship in today, the traditions we celebrate, and the spirit of Uttarakhand that continues to inspire millions. These are not mere historical records—they are the pulse of a living civilization that connects past, present, and future.
          </p>

          <Link href="/temples">
            <Button size="lg" className="px-8 mt-6 bg-gradient-to-r from-primary to-secondary hover:opacity-90 text-white">
